      ...
      .reduceByKey {
        ...
        case tupleWordUrlIdCount =>
          (tupleWordUrlIdCount._1, // Token
            ...
            math.log10(totalNumberOfDocuments / tupleWordUrlIdCount._2.toList.size), // IDF
            ...
            tupleWordUrlIdCount._2.toList.map(tupleUrlIdCount =>
              (tupleUrlIdCount._2._1, math.log(tupleUrlIdCount._2._2 + 1) * 
                  math.log10(totalNumberOfDocuments / tupleWordUrlIdCount._2.toList.size))).toMap)
      }

    println(finalRDD.count())
    finalRDD.repartition(10);
    println(finalRDD.count())
    val savingToCassandra = finalRDD.saveToCassandra("finder_keyspace", "inverted_index",
      SomeColumns("token_text", "collection_freq", "idf", "count_map", "tfidf_map"))
  }

  def isNotAStopWord(x: String): Boolean = {
    val stopwords = List("a", "about", "above", "above", "across", "after", "afterwards", "again", "against", "all", "almost",
    ...
    if (!stopwords.contains(x)) return true
    else return false
  }
}